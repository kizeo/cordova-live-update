  ...
  LiveUpdate = (function () {
    function LiveUpdate(options) {
      ...
      this.options = {
        ...
        afterDownloadComplete: (function (_this) {
          return function (current_id, latest_id) {
            ...
            res = confirm(
              sprintf(
                'Version %d is available for download (you are running %d). Update now?',
                latest_id,
                current_id,
              ),
            )
        ...
        getCurrentBuildId: (function (_this) {
          return function () {
            return Math.max(
              parseInt(localStorage.getItem(_this.options.localStorageVar)),
              _this.options.originalBuildId,
            )
          }
        })(this),
      ...
      if (!this.options.updateUrl || !this.options.originalBuildId) {
        ...
        throw new Exception(
          'LiveUpdater *requres* update URL and original build ID',
        )
      }
    }
          ...
          return _this
            ...
            ['finally'](function () {
              if (_this.keepChecking) {
                return setTimeout(
                  again,
                  timeoutMs || _this.options.recheckTimeoutMs,
                )
              }
            })
        }
          ...
          (function (_this) {
            return function (latest_build_id) {
              ...
              return _this.options
                .afterUpdateAvailable(current_build_id, latest_build_id)
                .then(function () {
                  return _this.download(latest_build_id)
                })
                .then(function () {
                  return _this.options.afterDownloadComplete(
                    current_build_id,
                    latest_build_id,
                  )
                })
                ...
                .then(function () {
                  return _this.options.afterInstallComplete(
                    current_build_id,
                    latest_build_id,
                  )
                })
      ...
      return this.checkOnce()
        ...
        .fail(
          (function (_this) {
            return function (err) {
              console.log(
                'Check failed, loading current local build of app.',
                err,
              )
    ...
    LiveUpdate.prototype.loadApp = function (build_id) {
      ...
      if (build_id) {
        new_app_html = sprintf(
          '%s%s/%s',
          this.options.bundleRoot,
          build_id,
          this.options.appEntryPoint,
        )
        return File.exists(new_app_html)
          .then(function () {
    ...
    LiveUpdate.prototype.download = function (build_id) {
      ...
      url = sprintf('%s/%s.zip', this.options.updateUrl, build_id)
